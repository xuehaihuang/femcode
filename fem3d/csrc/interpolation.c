...
/**
* \fn void interpVecP1toNcP1_3d(dCSRmat *P, ELEMENT_DOF *elementDOFp1, ELEMENT_DOF *elementDOFcr, FACE *faces)
...
void interpVecP1toNcP1_3d(dCSRmat *P, ELEMENT_DOF *elementDOFp1, ELEMENT_DOF *elementDOFcr, FACE *faces)
{
	...
	for (i = 0; i < elementDOFcr->dof; i++)
	{
		...
		curnode[2] = curnode[1] + elementDOFcr->dof;
		for (j = 0; j < 3; j++)
		{
			P->JA[P->IA[curnode[0]] + j] = faces->val[i][j];
			P->JA[P->IA[curnode[1]] + j] = faces->val[i][j] + elementDOFp1->dof;
			P->JA[P->IA[curnode[2]] + j] = faces->val[i][j] + elementDOFp1->dof*2;
		}
	}
