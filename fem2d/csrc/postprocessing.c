...
/**
 * \fn void projL2Pk2d(dvector *Qhu, ELEMENT *elements, dennode *nodes, ELEMENT_DOF *elementDOF, void (*u)(double *, double *, double *), double *paras)
	...
	for (k = 0; k < nt; k++){
		...
		for (i = 0; i<A.row; i++){
			...
			for (i1 = 0; i1<num_qp0; i1++){
				...
				u(x, val, paras);
				b.val[i] += s*weight0[i1] * val[0]*phi[0];
			} // i1
		} // k1

		init_dvector(&w, 0.0);
		den_pcg(&A, &b, &w, 10000, 1e-14, NULL, 0);
		for (i = 0; i < A.row; i++)
...
/**
 * \fn void projL2RM2d(dvector *Qhu, ELEMENT *elements, dennode *nodes, ELEMENT_DOF *elementDOF, void (*u)(double *, double *, double *), double *paras)
 * \brief L2 projection of the exact solution to the piecewise Lagrangian element space
 * \param *Qhu pointer to the L2 projection of u to the piecewise Lagrangian element space
 * \param *elements pointer to the structure of the triangulation
 * \param *nodes pointer to the nodes location of the triangulation
...
void projL2RM2d(dvector *Qhu, ELEMENT *elements, dennode *nodes, ELEMENT_DOF *elementDOF, void (*u)(double *, double *, double *), double *paras)
{
	...
	double phi1[2], phi2[2], val[2];
	int i, j, k, i1;
	double x[2], **vertices, s, **gradLambda, *edgeslength;
	short *eorien;
	...
	for (k = 0; k < nt; k++){
		...
		for (i = 0; i < A.row; i++){
			for (j = 0; j < A.col; j++){
				// A.val[i][j] = 0;
				for (i1 = 0; i1 < num_qp; i1++){
					nedelec1st_basis(lambdas[i1], gradLambda, edgeslength, eorien, i, 1, phi1);
					nedelec1st_basis(lambdas[i1], gradLambda, edgeslength, eorien, j, 1, phi2);
					A.val[i][j] += s*weight[i1] * (phi1[0] * phi2[0] + phi1[1] * phi2[1]);
				}
			}
		}
		...
		for (i = 0; i<A.row; i++){
			b.val[i] = 0;
			for (i1 = 0; i1<num_qp0; i1++){
				nedelec1st_basis(lambdas0[i1], gradLambda, edgeslength, eorien, i, 1, phi1);
				baryToCart2d(lambdas0[i1], x, vertices);
				u(x, val, paras);
				b.val[i] += s*weight0[i1] * (val[0]*phi1[0] + val[1]*phi1[1]);
			} // i1
		} // k1
